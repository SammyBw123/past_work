...
#vocabConVal, vocabConCount = np.unique(np.array(vocabConList),return_counts=True)
#vocabCon = dict(zip(vocabConVal,vocabConCount))
#vocabCon = OrderedDict(sorted(vocabCon.items(),key=lambda x:x[1],reverse=True))
...
#vocabLibVal, vocabLibCount = np.unique(np.array(vocabLibList),return_counts=True)
#vocabLib = dict(zip(vocabLibVal,vocabLibCount))
#vocabLib = OrderedDict(sorted(vocabLib.items(),key=lambda x:x[1],reverse=True))
